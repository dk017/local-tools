...
const StudioCanvas: React.FC<StudioCanvasProps> = ({
    ...
    useEffect(() => {
        if (onExportRef && stageRef.current) {
            onExportRef(() => {
                ...
                // Wait for render? No, sync usually works if state updates. 
                // But Transformer is a node. 
                // Actually, simpler: we can hide transformer node directly via ref if needed, 
                // or just rely on the fact that we clear selection.
                // However, setSelection might be async.
                ...
                // Restore?
                // if(oldSelected) setSelectedId(oldSelected); 
                // Let's keep it deselected for now
                return data;
            });
        }
    ...
    const handleTransformEnd = (e: any, id: string) => {
        ...
        // Reset scale to 1 and adjust width/height/fontSize for cleaner data
        // For Text, we usually scale fontSize. For Image, we keep scale or adjust width.
        // Konva recommends keeping scale for images, but for text updates it's better to update fontSize.

        // Simple approach: Store scale
        const newLayers = layers.map((layer) => {
            if (layer.id === id) {
                return {
                    ...layer,
