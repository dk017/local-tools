...
import {
    ...
    Download, MousePointer2, Move, ZoomIn, ZoomOut,
    Undo, Redo
} from 'lucide-react';
...
export default function StudioLayout({ initialBackground, initialDim, onExport }: StudioLayoutProps) {
    ...
    const addLayer = (type: 'text' | 'image' | 'rect' | 'circle', data: Partial<StudioLayerData>) => {
        ...
        // Circle x,y is center by default in Konva, so no offset needed if we want it at canvas center
    ...
    return (
        <div className="flex h-[85vh] bg-neutral-900 border border-white/10 rounded-xl overflow-hidden shadow-2xl">
            ...
            <div className="w-16 bg-black/40 border-r border-white/10 flex flex-col items-center py-4 gap-4 z-20">
                <SidebarIcon icon={<Layout size={20} />} label={t('templates')} active={activeTab === 'templates'} onClick={() => setActiveTab('templates')} />
                <SidebarIcon icon={<Type size={20} />} label={t('text')} active={activeTab === 'text'} onClick={() => setActiveTab('text')} />
                <SidebarIcon icon={<LucideImage size={20} />} label={t('photos')} active={activeTab === 'photos'} onClick={() => setActiveTab('photos')} />
                <SidebarIcon icon={<Square size={20} />} label={t('elements')} active={activeTab === 'elements'} onClick={() => setActiveTab('elements')} />
                <SidebarIcon icon={<Layers size={20} />} label={t('layers')} active={activeTab === 'layers'} onClick={() => setActiveTab('layers')} />
            </div>
            ...
            <div className="w-64 bg-neutral-900 border-r border-white/10 flex flex-col z-10">
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                    ...
                    {activeTab === 'photos' && (
                        <div className="space-y-4">
                            <div className="p-4 border-2 border-dashed border-white/10 rounded-lg text-center cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition">
